							...
							this.state.assets.map(
								asset =>
									<tr key={asset.id}>
										...
										<td>{asset.related_entities}</td>
										<td>{asset.status}</td>

										<td>
											<button className="btn btn-success" onClick={() => this.deleteAsset(asset.id)}> Delete</button></td>
										<td>
...
export default withRouter(ListAssetComponent);